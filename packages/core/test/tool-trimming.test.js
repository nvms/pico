...
test('context estimates shrink on trim, grow on restore, and yield to measured usage', () => {
  const usage = (tokens) => makeEvent('usage', { model: 'm', lastPrompt: tokens, usage: {} })
  const base = [...eventsFor({ description: 'write file', content: 'x'.repeat(20000) }, 'y'.repeat(20000)), usage(20000)]
  const trim = makeEvent('tool_trim', { callIds: ['c1'], version: { algorithm: 'tool-compact-v2' } })
  const trimmed = deriveState([...base, trim])
  assert.equal(trimmed.lastPromptTokens, 0)
  assert.equal(trimmed.lastPromptModel, 'm')
  assert.ok(trimmed.estimatedPromptTokens < 11000)
  assert.ok(trimmed.estimatedPromptTokens > 9000)
  const restore = makeEvent('tool_restore', { callIds: ['c1'] })
  assert.equal(deriveState([...base, trim, restore]).estimatedPromptTokens, 20000)
  const measured = deriveState([...base, trim, usage(10500)])
  assert.equal(measured.estimatedPromptTokens, null)
  assert.equal(measured.lastPromptTokens, 10500)
  assert.ok(deriveState([...base, trim, usage(10500), restore]).estimatedPromptTokens > 20000)
  assert.equal(deriveState([...base, trim, makeEvent('clear', {})]).estimatedPromptTokens, null)
})
